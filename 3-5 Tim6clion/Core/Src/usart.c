...
//串口1中断服务函数
void USART1_IRQHandler(void)//弱函数.weak USART1_IRQHandler
{
    HAL_UART_IRQHandler(&g_usart_handle); //会清除中断，类似
...
//     if (huart->Instance == USART1)
//     {
//         HAL_UART_Receive_IT(&g_usart_handle, g_rx_buffer, 11);  //  发送完成后，准备下一次接收
//         g_uart_1_rx_flag = 0;
//     }
// }